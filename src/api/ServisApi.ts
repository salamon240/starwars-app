...
export const fetchSearchResults = async (searchTerm: string, categories: { label: string; value: string }[]) => {
  try {
    const requests = categories.map(async (category) => {
      const response = await axios.get(`https://swapi.dev/api/${category.value}/?search=${searchTerm}`);
